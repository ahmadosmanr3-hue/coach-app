...
export default function BuilderPage() {
  ...
  function toggleExercise(exercise) {
    setDaysData(prev => {
      ...
      const exists = currentList.find(e => e.id === exercise.id)
      if (exists) {
        return {
          ...prev,
          [activeDay]: currentList.filter(e => e.id !== exercise.id)
        }
      }
      return {
        ...prev,
        [activeDay]: [...currentList, { ...exercise, sets: 3, reps: '10-12', rest: '60s' }]
      }
    })
  }
  ...
  async function generateAndSave() {
    ...
    const hasExercises = Object.values(daysData).some(d => d.length > 0)
    if (!hasExercises) {
      setStatus('Please select at least one exercise for any day.')
      return
    }
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans relative">
        ...
        <div className="lg:col-span-8 space-y-8">
          ...
          <section className="bg-slate-900/50 border border-slate-800 rounded-2xl p-6">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              ...
              <div>
                <label className="text-xs text-slate-500 mb-1 block flex items-center gap-1"><Dumbbell className="w-3 h-3" /> Goal</label>
                <input className="w-full bg-slate-950 border border-slate-800 rounded-lg px-3 py-2 text-sm outline-none"
                  value={client.goal} onChange={e => setClient({ ...client, goal: e.target.value })}
          ...
          <section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {filteredExercises.map(ex => {
                const isSelected = daysData[activeDay].some(s => s.id === ex.id)
                return (
                  <div
                    key={ex.id}
        ...
        <div className="lg:col-span-4 space-y-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 sticky top-24">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-lg font-semibold">Day {activeDay} Plan</h2>
              <span className="text-xs font-mono bg-slate-800 px-2 py-1 rounded-md">{daysData[activeDay].length} Exercises</span>
            </div>

            {daysData[activeDay].length === 0 ? (
              <div className="text-center py-10 text-slate-500 border-2 border-dashed border-slate-800 rounded-xl">
                <Dumbbell className="w-8 h-8 mx-auto mb-2 opacity-50" />
                <p className="text-sm">Select exercises for Day {activeDay}</p>
              </div>
            ) : (
              <div className="space-y-4 max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
                {daysData[activeDay].map((ex, idx) => (
                  <div key={ex.id} className="bg-slate-950 border border-slate-800 rounded-xl p-3 group text-slate-200">
                    ...
                    <div className="grid grid-cols-3 gap-2 mt-3 text-slate-100">
                      <div>
                        <label className="text-[10px] text-slate-500 uppercase tracking-wider">Rest</label>
                        <input className="w-full bg-slate-900 border border-slate-800 rounded px-2 py-1 text-xs text-center"
                          value={ex.rest} onChange={e => updateExerciseDetails(ex.id, 'rest', e.target.value)}
                        />
                      </div>
                      <div>
            ...
            <div className="mt-6 pt-6 border-t border-slate-800">
              ...
              <button
                ...
                disabled={saving || !Object.values(daysData).some(d => d.length > 0)}
      ...
      {/* PDF TEMPLATE (Tabular Style matching image) */}
      <div style={{ display: 'none' }}>
        <div ref={pdfRef} className="bg-white text-black p-12 w-[210mm] min-h-[297mm] font-sans">

          <div className="mb-10 mt-4">
            <h1 className="text-4xl font-extrabold text-black italic tracking-tighter mb-8">{courseName.toUpperCase() || 'FITNESS PLAN'}</h1>

            <div className="grid grid-cols-2 gap-x-12 gap-y-2 text-sm">
              <div className="flex border-b border-black pb-1">
                <span className="font-bold mr-2">NAME:</span>
                <span className="flex-1 underline">{client.name}</span>
              </div>
              <div className="flex border-b border-black pb-1">
                <span className="font-bold mr-2">HEIGHT:</span>
                <span className="flex-1 underline">{client.height}</span>
              </div>
              <div className="flex border-b border-black pb-1">
                <span className="font-bold mr-2">AGE:</span>
                <span className="flex-1 underline">{client.age}</span>
              </div>
              <div className="flex border-b border-black pb-1">
                <span className="font-bold mr-2">WEIGHT:</span>
                <span className="flex-1 underline">{client.weight}</span>
              </div>
              <div className="flex border-b border-black pb-1 col-span-2">
                <span className="font-bold mr-2">GOAL:</span>
                <span className="flex-1 underline">{client.goal}</span>
              </div>
            </div>
          </div>
